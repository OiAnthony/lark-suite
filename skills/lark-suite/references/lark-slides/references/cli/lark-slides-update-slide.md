...
和其他整页写入一样，把 `--content` 存成本地文件后先跑版式 lint。先取得当前已加载 `lark-slides/GUIDE.md` 的父目录，记为 `<lark-slides-skill-dir>`；不要猜测全局安装路径：
...
按当前已加载 `lark-slides/GUIDE.md` 指向的 [validation-xml.md](../workflow/validation-xml.md) 完成验证：核对总页数、目标页和关键元素（包括需要保留的 ID、文本、背景与备注），并对回读 XML 运行同一版式 lint；发现差异时先停止后续写入并重新基于最新版处理。
