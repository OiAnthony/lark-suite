...
执行本总框架内的 workflow 前，必须先阅读 [`../../lark-shared/GUIDE.md`](../../lark-shared/GUIDE.md) 了解认证、全局参数和安全规则。
...
|-------|------|--------------|
| `lark-drive/GUIDE.md` | 用户意图到具体 workflow entry 的短路由 | 长流程逻辑、未注册场景 |
...
- 只有 workflow 已经具备可执行规则时，才允许作为本总框架 workflow 出现在 `GUIDE.md` 并加入 `Workflow Registry`。
