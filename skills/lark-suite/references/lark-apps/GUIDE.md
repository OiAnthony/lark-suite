...
妙搭应用属于用户资产。默认用 `--as user`；认证、scope、exit-10、高风险确认、`_notice` 等通用处理只读 [`../lark-shared/GUIDE.md`](../lark-shared/GUIDE.md)，不要在本 skill 里复制。妙搭应用有两条开发路径：**本地开发**（拉源码本地写）/ **云端会话**（妙搭 AI 生成）。
...
因缺权限失败（`error.subtype == "missing_scope"`）时的通用处理见 [`../lark-shared/GUIDE.md`](../lark-shared/GUIDE.md)，同样按 `--domain apps` 授权；授权成功后只恢复原业务操作，不扩展任务范围。
...
| 创意模式（html）应用的评论相关操作 | 创意模式应用评论走 lark-drive 文档评论体系，读取 [`../lark-drive/GUIDE.md`](../lark-drive/GUIDE.md) 了解评论能力 | [`../lark-drive/GUIDE.md`](../lark-drive/GUIDE.md) |
